...
### v6.5 fixes
- **TOC bug**: после перехода по оглавлению (PDF/текст) кнопки и свайп снова работают — рендер всегда mode-aware.
- **2 страницы / горизонтально**: настоящий spread (две колонки) без CSS multi-column; горизонталь через scroll-snap.
- **Настройки везде**: кнопка ⚙ в библиотеке и в ридере; ⭐ быстрый фильтр избранного в шапке библиотеки.
- **Избранное**: звезда на карточках + фильтр «⭐ Избранное» + кнопка в шапке; теги в настройках открытой книги.
- **PDF подсветка TTS**: улучшенный sequential matching по text-layer spans.
- **Абзацы**: более надёжное разбиение (fallback на одиночные переносы строк).
- **Голоса RU**: сортировка по качеству/локальности, подсказка если системных русских голосов мало (iOS).
